...
class HuffmanSuite extends FunSuite {
  ...
  test("singleton") {
    ...
    assert(!singleton(List(Leaf('a', 2), Leaf('b', 1))))
  }
  ...
  test("combine of ordered leaf list") {
  ...
  test("until") {
    val leaflist = List(Leaf('e', 1), Leaf('t', 2), Leaf('x', 4))
    val x = until(singleton, combine)(leaflist)
    println(x)
  }
  
  test("createCodeTree") {
    val tree = createCodeTree(List('h', 'e', 'l', 'l', 'o', ',', ' ', 'w', 'o', 'r', 'l', 'd'))
    println(tree)
  }
